...
def img_Transform(car_contours, oldimg, pic_width, pic_hight):
    car_imgs = []
    for car_rect in car_contours:  # [((264.3843688964844, 259.8280334472656), (35.244625091552734, 134.99066162109375), -89.32596588134766)]
        if -1 < car_rect[2] < 1:
            angle = 1
            # 对于角度为-1 1之间时，默认为1
        else:
        ...
        # x1 = int(car_rect[0][0])
        # y1 = int(car_rect[0][1])
        # x2 = x1 + int(car_rect[1][0])
        # y2 = y1 + int(car_rect[1][1])
        # car_img = oldimg[x1:x2, y1:y2]
        if left_point[1] < right_point[1]:  # 正角度
            ...
            # print('left_point heigth_point left_point new_right_point')
        ...
        # dst = cv2.warpAffine(oldimg, M, (pic_width, pic_hight))
            ...
            # print('right_point heigth_point new_left_point right_point')
            # print(int(right_point[1]), int(heigth_point[1]), int(new_left_point[0]), int(right_point[0]))  # 172 172 20 172
            car_imgs.append(car_img)
